...
func TestOpenTofuPlanRunsThroughSchedulerAndPersistsArtifacts(t *testing.T) {
	initACLTestDB(t)
	...
	if logs, err = db.JobLogsForJob(job.ID); err != nil || len(logs) == 0 {
		t.Fatalf("runner logs missing: %#v err=%v", logs, err)
	}
	for _, log := range logs {
		if strings.Contains(log.Message, "runner-secret") {
			t.Fatal("secret reached stored job logs")
		}
	}
}
