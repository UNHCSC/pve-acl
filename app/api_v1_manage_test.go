...
func TestAdminCanCreateAndListCloudGroups(t *testing.T) {
	...
	var (
		token      string     = authenticateTestUser(t, "group-admin", true)
...
func TestRoleBoundAdminCanCreateCloudGroups(t *testing.T) {
	...
	if dbUser, _, err = db.EnsureUser("role-admin", "Role Admin", "role-admin@example.test", "local", "role-admin"); err != nil {
...
func findTestGroupBySlug(t *testing.T, slug string) (cloudGroupResult *db.CloudGroup) {
	t.Helper()
...
func TestGroupManagerCanManageMembershipRoles(t *testing.T) {
	...
	if group, _, err = db.EnsureCloudGroup("Project Team", "project-team", db.GroupTypeProject); err != nil {
		t.Fatalf("EnsureCloudGroup returned error: %v", err)
	}

	if manager, _, err = db.EnsureUser("group-manager", "Group Manager", "manager@example.test", "local", "group-manager"); err != nil {
		t.Fatalf("EnsureUser manager returned error: %v", err)
	}

	if target, _, err = db.EnsureUser("group-target", "Group Target", "target@example.test", "local", "group-target"); err != nil {
	...
	if membership, found, err = db.CloudGroupMembershipForUserAndGroup(target.ID, group.ID); err != nil || !found {
...
func TestGroupManagerCanPatchMembershipRole(t *testing.T) {
	...
	var (
		...
		target     *db.User
		membership *db.CloudGroupMembership
		updated    *db.CloudGroupMembership
		token      string
		fiberApp   *fiber.App
		req        *http.Request
		resp       *http.Response
	...
	if group, _, err = db.EnsureCloudGroup("Role Patch Group", "role-patch-group", db.GroupTypeProject); err != nil {
		t.Fatalf("EnsureCloudGroup returned error: %v", err)
	}

	if manager, _, err = db.EnsureUser("group-role-manager", "Group Role Manager", "manager@example.test", "local", "group-role-manager"); err != nil {
		t.Fatalf("EnsureUser manager returned error: %v", err)
	}

	if target, _, err = db.EnsureUser("group-role-target", "Group Role Target", "target@example.test", "local", "group-role-target"); err != nil {
		t.Fatalf("EnsureUser target returned error: %v", err)
	}
	...
	if updated, found, err = db.CloudGroupMembershipForUserAndGroup(target.ID, group.ID); err != nil || !found {
...
func TestAdminCanUpdateAndArchiveCloudGroup(t *testing.T) {
	...
	var (
		group      *db.CloudGroup
		updated    *db.CloudGroup
		archived   *db.CloudGroup
		token      string
		fiberApp   *fiber.App
	...
	if group, _, err = db.EnsureCloudGroup("Archive Me", "archive-me", db.GroupTypeCustom); err != nil {
		t.Fatalf("EnsureCloudGroup returned error: %v", err)
	}
	...
	if updated, found, err = db.GetCloudGroupByID(group.ID); err != nil || !found {
	...
	if archived, found, err = db.GetCloudGroupByID(group.ID); err != nil || !found {
...
func TestAdminCanCreateCustomRoleAndGrantPermission(t *testing.T) {
	...
	var (
		token      string     = authenticateTestUser(t, "role-catalog-admin", true)
		fiberApp   *fiber.App = newAuthenticatedFiberApp()
		createReq  *http.Request
		createResp *http.Response
...
func TestAdminCanUpdateAndDeleteUnboundCustomRole(t *testing.T) {
	...
	if role, _, err = db.EnsureRole("TemporaryOperator", "Temporary role", false); err != nil {
	...
	if updated, found, err = db.GetRoleByName("TemporaryViewer"); err != nil || !found {
...
func TestSystemRoleEditAndDeleteAreProtected(t *testing.T) {
	...
	var (
		role       *db.Role   = findTestRoleByName(t, db.DefaultLabAdminRoleName)
		token      string     = authenticateTestUser(t, "system-role-admin", true)
		fiberApp   *fiber.App = newAuthenticatedFiberApp()
		patchReq   *http.Request
		patchResp  *http.Response
		deleteReq  *http.Request
...
func TestBoundCustomRoleCannotBeDeleted(t *testing.T) {
	...
	if role, _, err = db.EnsureRole("StillInUse", "Bound role", false); err != nil {
		t.Fatalf("EnsureRole returned error: %v", err)
	}

	if user, _, err = db.EnsureUser("bound-role-user", "Bound Role User", "bound@example.test", "local", "bound-role-user"); err != nil {
		t.Fatalf("EnsureUser returned error: %v", err)
	}
...
func TestGroupRoleBindingRequiresScopedRoleManage(t *testing.T) {
	...
	var (
		...
		globalResp *http.Response
		scopedBody string
		scopedReq  *http.Request
		scopedResp *http.Response
		err        error
	)

	if group, _, err = db.EnsureCloudGroup("Scoped Group", "scoped-group", db.GroupTypeProject); err != nil {
		t.Fatalf("EnsureCloudGroup returned error: %v", err)
	}

	if user, _, err = db.EnsureUser("scoped-role-admin", "Scoped Role Admin", "scoped@example.test", "local", "scoped-role-admin"); err != nil {
		t.Fatalf("EnsureUser returned error: %v", err)
	}
...
func TestGroupOwnerCanManageOnlyOwnScopedRoleBindings(t *testing.T) {
	...
	var (
		...
		wrongGroupReq  *http.Request
		wrongGroupResp *http.Response
		deleteReq      *http.Request
		deleteResp     *http.Response
		err            error
	)

	if group, _, err = db.EnsureCloudGroup("Owner Scoped Group", "owner-scoped-group", db.GroupTypeProject); err != nil {
		t.Fatalf("EnsureCloudGroup returned error: %v", err)
	}
	if otherGroup, _, err = db.EnsureCloudGroup("Other Scoped Group", "other-scoped-group", db.GroupTypeProject); err != nil {
		t.Fatalf("EnsureCloudGroup other returned error: %v", err)
	}
	if owner, _, err = db.EnsureUser("group-owner", "Group Owner", "owner@example.test", "local", "group-owner"); err != nil {
		t.Fatalf("EnsureUser returned error: %v", err)
	}
	...
	if bindings, err = db.RoleBindingsForSubject(db.RoleBindingSubjectGroup, group.ID); err != nil {
...
func findTestRoleByName(t *testing.T, name string) (roleResult *db.Role) {
	t.Helper()
	...
	if roles, err = db.Roles.SelectAll(); err != nil {
