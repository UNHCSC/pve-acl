...
func TestLDAPSyncOnlyImportsConfiguredAndOptInGroups(t *testing.T) {
	...
	var (
		dbUser       *db.User
		syncedGroup  *db.CloudGroup
		domainAdmins *db.CloudGroup
		groups       []*db.CloudGroup
		found        bool
		err          error
	)

	if dbUser, _, err = db.EnsureUser("ldap-sync-user", "LDAP Sync User", "sync@example.test", "ldap", "ldap-sync-user"); err != nil {
		t.Fatalf("EnsureUser returned error: %v", err)
	}
	if syncedGroup, _, err = db.EnsureCloudGroup("Teaching Staff", "teaching-staff", db.GroupTypeCustom); err != nil {
...
func TestRequireAPIAuthSetsCurrentUser(t *testing.T) {
	...
	if user, err = auth.Authenticate("middleware-user", "secret"); err != nil {
